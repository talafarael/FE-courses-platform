...
export const useFindCurrentSection = () => {
  ...
  const [currentSection, setCurrentSection] = useState<LectureSection | ITest>()
  const params = useParams<{ id: string; "entries-id": string, "section-id": string }>();
    ...
    currentCurse?.units?.forEach((unit) => {
      unit?.entries?.forEach((entry: IEntries) => {
        ...
        if (entry.id === entriesId) {
          setCurrentUnit(ent);
          if (ent?.test) {
            const section = ent.test.sections.find((elem) => elem?.id == params?.["section-id"])
            if (section) setCurrentSection(section)
          }
          if (ent?.lecture) {

            const section = ent.lecture.sections.find((elem) => elem?.id == params?.["section-id"])
