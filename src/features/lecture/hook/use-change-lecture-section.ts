...
export const useChangeLectureSection = () => {
  ...
  const changeLectureSection = useCallback(
    async (data: ICreateLectureSectionForm, sectionId: string) => {
      if (!params) return;

      const { id, "entries-id": entriesId, "lecture-id": lectureId } = params;
      const res = await handlerSubmit(() =>
        changeLectureSectionMutation(
          {
            ...data,
            id: sectionId,
          },
      ...
      //router.push(
      //  ROUTES.redactor(id, entriesId, res?.data?.data?.data?.lecture_id),
      //);
    },
